...
export const ResourceProvider = ({ children }) => {
  ...
  const fetchResources = async (userRole) => {
    ...
    try {
      ...
      let endpoint = "http://localhost:3000/api/resource/public";
      const config = {};

      if (userRole === "student") {
        endpoint = "http://localhost:3000/api/resource/student";
        config.headers = { Authorization: `Bearer ${token}` };
      } else if (userRole === "faculty") {
        endpoint = "http://localhost:3000/api/resource/faculty";
        config.headers = { Authorization: `Bearer ${token}` };
      }

      const res = await axios.get(endpoint, config);
      setResources(res.data.payload || []);
      console.log(`Loaded resources for role: ${userRole}`, res.data.payload);
    } catch (error) {
